...
def get_current_matchday_info():
    response = requests.get(f"{football_adapter_service_url_base}/matchday_info?matchday=25") # TEST: ?matchday=21
    if response.status_code != 200:
        return {"error": "unable to fetch current matchday info"}
    
    return response.json()

@app.get("/{league_id}", response_model=League, summary = "Get a League")
...
@app.get("/{league_id}/logged-owner", response_model=IsOwner, summary = "Is the logged user the owner of the League") 
...
@app.get("/{league_id}/table", response_model=list[TableSquadInfo], summary = "Get the Table/Board of the league so far") # classifica della lega
...
@app.post("/{league_id}/participants", status_code=201, summary = "Add a participant to the League", response_model = LeagueWithParticipants) 
